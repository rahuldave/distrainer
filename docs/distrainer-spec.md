...
deploy/
  ...
  drivers/uncloud.sh   # uncloud: Docker hosts joined by a WireGuard mesh (DISTRAINER_DRIVER=uncloud, section 11)
  uncloud/             # compose.yml the uncloud driver deploys; machines.sh, the OrbStack machine bootstrap
  Dockerfile
  docker-compose.yml
...
**Driver abstraction.** Nothing outside `deploy/` knows how containers are started. `deploy/driver.sh <verb> [args]` dispatches to `deploy/drivers/<DISTRAINER_DRIVER>.sh` (default `compose`) and the verbs are the whole contract: `build`, `up N [minio]`, `down`, `nuke`, `wipe-shared`, `scale N`, `exec-head CMD...`, `kill-worker I`, `kill-head`, `stop-worker I`, `cp-from-head SRC DST`, `shared`, `endpoint`, `mkbucket`. The Justfile harness targets and `integration_tests/cluster/run_scenarios.py` call only verbs, so the same scenarios run on OrbStack today, on KubeRay (`DISTRAINER_DRIVER=kuberay`, section 11), on an uncloud cluster (`DISTRAINER_DRIVER=uncloud`, section 11: `uc deploy`/`uc scale`/`uc exec` over a WireGuard mesh of Docker hosts), and on RunPod GPU pods (`DISTRAINER_DRIVER=runpod`, section 11: the REST v2 API, one pod per node over global networking). A driver may add verbs of its own (`kuberay`: `operator`, `submit`; `uncloud`: `machines-up`, `machines-status`, `machines-destroy`; `runpod`: `cost`, `catalog`); the Justfile and the runner use only the common set. A driver without a volume that spans its nodes prints nothing for `shared`, and the runner then reads the audit trail and the checkpoints from the S3 store through the driver's `endpoint` (S2, S3, S4 and S8 run on the bucket; S6 and S11, whose configs store under `/shared`, are skipped).
...
## 11. Phase 2: KubeRay on OrbStack Kubernetes (M5) and uncloud machines (M6)
...
**uncloud (M6, mode C of `docs/running-modes.md`).** `deploy/drivers/uncloud.sh` implements the driver verbs on the `uc` CLI over a cluster of Docker hosts joined by uncloud's WireGuard mesh; `deploy/uncloud/compose.yml` declares the cluster (the same node image, scripts, Ray environment, shared memory and health checks as `docker-compose.yml`, pinned against it by `tests/test_deploy_manifests.py`: `head` and `minio` pinned to the head machine with `x-machines`, `worker` replicas to the other machines, `deploy.replicas` from `up N`, ports published with `x-ports ...@host` only inside a host prefix, no build and no bind mounts because nothing spans machines: `build` pushes the local image with `uc image push`, `pull_policy: never`). `deploy/uncloud/machines.sh` builds the local test bed: two or three OrbStack Linux machines (`orb create` with memory and CPU limits, the head machine larger: it runs the Ray head, the Train controller, the driver script and MinIO and thrashed under 3 and 4 GB), `uc machine init` / `uc machine add` over OrbStack's `ucN@orb` ssh route (uncloud runs the system `ssh`), each machine's address on OrbStack's machine network as an explicit WireGuard endpoint, ingress off, the cluster network checked against OrbStack's for overlap. Storage is S3-compatible only (MinIO on the head machine; the harness configs' `http://minio:9000` resolves cluster-wide because uncloud gives every container `search internal.`); `shared` prints nothing and the scenario runner reads the bucket through `endpoint`. Node death is `docker kill` over ssh on the machine that runs the container (uncloud has no per-container kill), restarted after `DISTRAINER_RESTART_DELAY` as in compose; `scale` is `uc scale`, which stops a removed worker with a 10 s grace (a preemption notice); `up` after `kill-head` recreates the head and the workers reconnect. Every `uc` call names its context, a failing or reshaped listing fails the verb, `down` removes every copy of a service by id (uncloud can register a name twice). Acceptance on 2026-09-14 with `run_scenarios.py` reading the bucket and `check_audit.py` unchanged: S2, S3, S4, S8, S9, S10, S11s3 green under `DISTRAINER_DRIVER=uncloud` (S6 and S11 skip); `docs/uncloud-gotchas.md` records what the tool does that the driver works around. Real cloud machines came in M7, below.

**The AWS bed (M7).** `deploy/uncloud/aws.sh` is the EC2 twin of `machines.sh` (`up | status | stop | start | destroy | bucket | bucket-rm | env`, driven through the `machines-*` verbs under `DISTRAINER_UNCLOUD_PROVIDER=aws`): a key pair, a security group that admits ssh from the operator's address only and WireGuard from the group itself (the dashboard through an ssh tunnel), three tagged Graviton instances in a zone that offers the types, the uncloud context joined over the private addresses, park and resume with the new public addresses rewritten into a generated ssh config and the uc context, and an S3 bucket with an IAM user scoped to it; `.harness/aws/env` carries the context, the machines, the ssh route (`DISTRAINER_UNCLOUD_SSH_OPTS`), the head's public address and the `S3_*` settings; the uncloud driver and the scenario runner read it after `.env` through `DISTRAINER_ENV_FILE` (skipped when the caller pins another bed), the caller's environment winning over both files. With `S3_ENDPOINT` naming a store outside the cluster the driver's `endpoint` prints `s3=<URL>` instead of `minio=...`, and the runner's store becomes `harness-s3.yaml`'s bucket: no MinIO deployed, no bucket made, S9/S10/S11s3 parametrised by the store. Acceptance on 2026-09-16 (`t4g.large` head, two `t4g.medium` workers in us-east-1b, the bucket in us-east-1): S2 100 s, S3 103 s, S4 179 s, S8 87 s, S9 197 s, S10 163 s, S11s3 220 s, all green (OrbStack: 149, 146, 210, 125, 254, 205, 161). Two behaviours of a real object store shaped the runner: the Mac-side client signs by region, and the Train controller registers checkpoints a couple of reports behind the workers (its bookkeeping is S3 round trips), so the replay bound of section 10 takes a `lag_intervals` allowance on a store outside the cluster (24 and 18 positions replayed against 11 and 14; the ledger keeps every position trained). Checkpoint latency sets the step pace there (0.67 s per step with `every_k: 2` against 0.25 s on MinIO), which inverts the S11 pacing premise; a policy that adapts the checkpoint interval to the store is the pointer for the next stage.

**Phase 7b: the image in a private registry, an x86 bed.** The uncloud driver's `build` takes a registry image (`DISTRAINER_IMAGE` naming a host): one `docker buildx build --platform linux/amd64,linux/arm64 --push` on a `docker-container` builder it creates once, then every machine logs in over the ssh route with the Mac's registry token (ECR, twelve hours; uncloud has no registry login) and pulls in-region; a local image keeps `docker build` plus `uc image push`. `aws.sh ecr` makes the tagged private repository with a lifecycle policy and records it, so the env file names the image and the platforms; the bed defaults to x86 (`t3.large`, `t3.medium`), Graviton staying a setting. Acceptance on 2026-09-16 on the x86 bed pulling from ECR: S2 107 s, S3 118 s, S4 193 s, S8 98 s, S9 212 s, S10 161 s, S11s3 222 s, all green, within ten percent of the arm64 bed; the push of both halves took ten minutes from the Mac once, the pulls seconds. Tutorial 5 documents both beds and which image to build for which machine.

**Mode E: RunPod pods (M8).** `deploy/drivers/runpod.sh` (`DISTRAINER_DRIVER=runpod`) puts one GPU pod per Ray node on RunPod's secure cloud, joined by global networking (`<pod id>.runpod.internal`, TCP), created from the GPU image of `deploy/Dockerfile.gpu` (the CUDA wheels of the locked torch over the CPU image's layout, sshd from RunPod's injected key; built by GitHub Actions into GHCR) through the REST v2 API. The head is a GPU pod too (a CPU pod cannot join global networking) and advertises no `trainer`; nothing spans pods, so the store is the S3 bucket and `shared` prints nothing; the driver acts only on pods named `<cluster>-head` / `<cluster>-worker-N` that carry `DISTRAINER_CLUSTER=<cluster>`; v2 takes one GPU type per create, so an ordered list is tried under a spend guard (`DISTRAINER_RUNPOD_MAX_GPU_HOURLY`); `kill-worker` terminates the pod and creates a new one of the same name after the delay, `kill-head` makes the next `up` replace the workers too (they dial the head by its pod id); pods are terminated, never stopped. Tutorial 6 (`docs/tutorials/runpod.md`) is the guide; `examples/image_contrastive` (section 8) is the workload.
...
3. **M3 — harness**: driver interface (`deploy/driver.sh`, compose driver, an uncloud stub implemented in M6), Dockerfile, compose, Justfile targets, `run_scenarios.py` + `check_audit.py`; S2, S3, S4 on local shared storage, then S9, S10 against MinIO. Test strategy: characterization-first (record the audit logs of a green run, then assert).
...
6. **M6 — uncloud variant** (mode C): `deploy/uncloud/` bootstraps 2 to 3 small OrbStack Linux machines into an uncloud cluster (Docker hosts joined by uncloud's WireGuard mesh, which rides on OrbStack's own machine network), `deploy/drivers/uncloud.sh` implements the same verbs on S3-only storage (no volume spans machines: `shared` prints nothing), and the scenario runner works without a shared mount; S2, S3, S4, S8 (on MinIO), S9, S10, S11s3 green under `DISTRAINER_DRIVER=uncloud` with `check_audit.py` unchanged, S6 and S11 skipped (section 11). Test strategy: characterization-first against the existing checks.
7. **M7 — the AWS bed (real machines with S3)**: `deploy/uncloud/aws.sh`, the drivers' `DISTRAINER_ENV_FILE`, the runner's store outside the cluster, tutorial 5. Acceptance: S2, S3, S4, S8, S9, S10, S11s3 green on EC2 against S3 (section 11). Test strategy: characterization-first against the existing checks, which is what surfaced the region and the controller-lag behaviours. **M7b**: the image in a private registry (ECR) as a multi-architecture manifest, the bed on x86; the same scenarios green there (section 11).
8. **M8 — a GPU contrastive example on RunPod pods** (done, PR #21): `examples/image_contrastive` (SimCLR on CIFAR-10 blocks with a kNN probe, section 8), `deploy/Dockerfile.gpu` built by GitHub Actions into GHCR, `deploy/drivers/runpod.sh` on the REST v2 API (global networking, a spend guard, a session stop mode with drained Ray nodes; section 11 mode E), tutorial 6. Acceptance met: the example on one GPU with the probe rising, on two pods, with a worker lost mid-run; S2, S3, S4, S9, S10 green on pods. Next (M9, `internal_docs/handoff-m9.md`, `docs/parallelism.md`): a real process group in the tests, a `parallel:` section, a sync point on every rank at the segment end for local SGD and DiLoCo, sharded checkpoints for FSDP.
9. **M9 — the parallel kinds on the block loop, proven on the CPU** (done, PR #24): `tests/procgroup.py` runs `n` ranks as processes over a Gloo group; the `parallel:` section (`ddp` | `none` | `local_sgd` | `diloco` | `fsdp`, section 7); the segment-end sync on every rank (`distrainer/parallel.py`, section 4) with DiLoCo's outer optimizer in the checkpoint; FSDP2 wrapping and the sharded checkpoint shape (section 6.2) with resharding on load; the docs site (`docs/` on GitHub Pages) and `internal_docs/`. Acceptance met on the CPU: every kind under real ranks in the tests and on a local Ray cluster; the image example under `fsdp` equal to `ddp`. The GPU runs (DiLoCo across the transatlantic pods, FSDP on a multi-GPU pod) and the driver's per-pod GPU count are M10 (`internal_docs/handoff-m10.md`).
...
3. Run `gsu` with the **python-uv** language profile: `pyproject.toml` managed by `uv`, `.gitignore` from `base + python-uv` templates, `.envrc`/`.env.example` (S3 credentials for the harness live here, never committed), and the `Justfile` command contract below. Copy `internal_docs/gest_codex_workflow.md` from the skills repo into `docs/` (the installer does not populate `docs/`).
...
- **Repository**: `github.com/rahuldave/distrainer`, public, MIT. Every milestone M0–M6 is a "major task" with its own GitHub issue; leaf tasks are Gest-only and carry `parent_task` pointing at the milestone task that is paired with the issue.
- **Harness driver**: compose on OrbStack (the default), KubeRay on OrbStack's Kubernetes (M5, section 11) and uncloud on OrbStack machines (M6, section 11) are the implemented drivers, one verb interface. Nothing in `distrainer/`, `tests/`, or `integration_tests/` may depend on the driver.
