...
Two things follow from "written once". First, segments are always complete before anyone trains on them: the writer stores all `W` block files, then writes the segment file in one atomic step, and the trainer only reads segment files that exist. So when training starts on segment 7, the full list of segment 7 is already on disk, which is what makes resuming after a crash well defined in both modes. Second, in batch mode the grouping is not fixed across epochs: for each pass over the data the writer shuffles the *whole* list of blocks and only then cuts it into segments, so which blocks share a segment, as well as their order, changes from one pass to the next. Blocks themselves stay the same unless a hook rebuilds them.

That one structure covers both ways data can arrive. In **batch mode**, all of the data exists before training starts, so the whole log is written up front, every segment at once, and an `_END` marker says "that is everything". In **streaming mode**, blocks are being produced while training runs (by a miner that keeps finding new hard negatives, say), so segments are appended as they fill up and there is no `_END` until the producer stops. The trainer does not care which mode it is in. It reads segment 0, then segment 1, and when it reaches a segment that does not exist yet it waits for it to appear. That waiting is the only difference between the two modes, and it is invisible to the training code. It does mean `W` is also the streaming delay: a segment cannot be trained on until `W` blocks have arrived, so streaming logs usually use a smaller `W`. And because arrival order decides which blocks share a segment, a streaming writer that wants more mixing can hold a buffer of several segments' worth of blocks and fill each segment by sampling from it.
...
It is worth being explicit about what this is *not*, because it is the opposite of Part 5. Nothing is **yielded**. There is no generator, iterator, queue or coordinator handing batches to the ranks, and the producer never calls into the trainer or the other way round. The producer's whole job is to **write**: it materialises every block as a Parquet file under `blocks/`, and when it has `W` of them it appends one small commit record to the log, the segment file, naming those blocks in their shuffled order. That is the same shape as a Delta Lake or Iceberg table: data files plus an append-only transaction log, where a write is "new data files, then one atomic log entry", and readers replay the log to learn what exists. The trainer is such a reader. Each rank opens the log, finds the segment it should be on, fetches its own blocks by name, and moves to the next segment file when it exists. Everything that follows in this document comes from that choice: a run can be stopped and resumed at an exact position because a position is a place in a file, not a place in someone's iterator; the world size can change because re-dealing a segment is re-reading a list; the producer can run anywhere that can write to the store, before the trainer, during it, or long after it has finished; several runs can train on the same log; and the only thing that bounds a log's size is the garbage collector, not back-pressure, because a writer that gets ahead simply writes more files.
