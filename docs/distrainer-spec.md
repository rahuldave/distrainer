...
8. **M8 — a GPU contrastive example on RunPod pods** (done, PR #21): `examples/image_contrastive` (SimCLR on CIFAR-10 blocks with a kNN probe, section 8), `deploy/Dockerfile.gpu` built by GitHub Actions into GHCR, `deploy/drivers/runpod.sh` on the REST v2 API (global networking, a spend guard, a session stop mode with drained Ray nodes; section 11 mode E), tutorial 6. Acceptance met: the example on one GPU with the probe rising, on two pods, with a worker lost mid-run; S2, S3, S4, S9, S10 green on pods. Next (M9, `docs/handoff-m9.md`, `docs/parallelism.md`): a real process group in the tests, a `parallel:` section, a sync point on every rank at the segment end for local SGD and DiLoCo, sharded checkpoints for FSDP.
